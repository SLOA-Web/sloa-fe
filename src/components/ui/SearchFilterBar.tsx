...
const SearchFilterBar: React.FC<SearchFilterBarProps> = ({
  ...
  return (
    <div className={`bg-gradient-to-r from-white/95 to-white/90 backdrop-blur-lg rounded-2xl shadow-md border border-primary/20 p-6 mb-12 ${className}`}>
      <div className={`flex flex-col lg:flex-row lg:items-center gap-6 lg:justify-between`}>
        ...
        <div className="flex flex-col items-center lg:items-end">
          <span className="text-xs font-medium text-gray-600 mb-1 font-roboto uppercase tracking-wide">
            Total Results
          </span>
          <div className="text-lg font-bold text-primary font-roboto bg-gradient-to-r from-primary/10 to-primary/5 px-4 py-3 rounded-[8px] border-2 border-primary/20">
            {totalResults} <span className="text-sm font-medium text-primary/80 font-poppins">{resultsLabel}</span>
          </div>
        </div>
      </div>
    </div>
