    ...
    <section className="py-12 lg:py-24 px-4 md:px-10 lg:px-16">
      <div className="container ">
        <div className="relative flex flex-col lg:flex-row gap-8 lg:gap-16">
          ...
          <div className="order-2 lg:order-1 lg:w-1/2 lg:mb-12 lg:pr-12">
            ...
            <div className="space-y-6">
              {content.map((paragraph, index) => (
                <p
                  key={index}
                  className="text-[#122D1E] font-poppins text-[16px] md:text-[20px] lg:text-[24px] leading-7"
                >
                  {paragraph.split("\n").map((line, i, arr) =>
                    i < arr.length - 1 ? (
                      <span key={i}>
          ...
          {/* Right Column - Image */}
          <div className="order-1 lg:order-2 lg:absolute lg:top-0 lg:right-0 lg:w-1/2 lg:h-full">
            <SimpleImage
              src={imageUrl}
              alt={imageAlt}
